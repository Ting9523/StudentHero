...
class WP_Data_Access_Admin {
	...
	public function enqueue_styles() {
		...
		if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
			// Premium dashboard.
			wp_register_style(
				'wpdataaccess_dashboard_premium',
				plugins_url( '../assets/premium/css/wpda_dashboard.css', __FILE__ ),
				array(),
				WPDA::get_option( WPDA::OPTION_WPDA_VERSION )
			);
			if ( self::PAGE_DASHBOARD === $this->page ) {
				wp_enqueue_style(
					'wpdataaccess_dashboard_premium_restyle',
					plugins_url( '../assets/premium/css/wpda_dashboard_restyle.css', __FILE__ ),
					array(),
					WPDA::get_option( WPDA::OPTION_WPDA_VERSION )
				);
			}
		...
		if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
			...
			\WPDataAccess\Premium\WPDAPRO_Inline_Editing\WPDAPRO_Inline_Editing_Init::enqueue_styles( $this->page );
		...
		if (
				self::PAGE_DASHBOARD === $this->page ||
				self::PAGE_QUERY_BUILDER === $this->page ||
				self::PAGE_PUBLISHER === $this->page ||
				self::PAGE_CHARTS === $this->page
		) {
		...
		if ( self::PAGE_QUERY_BUILDER === $this->page ) {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				wp_enqueue_style(
					'wpda_visual_query_builder',
					plugins_url( '../assets/premium/css/wpda_query_builder.css', __FILE__ ),
					array(),
					WPDA::get_option( WPDA::OPTION_WPDA_VERSION )
				);
			}
	...
	public function enqueue_scripts() {
		...
		if ( self::PAGE_PUBLISHER === $this->page ) {
			if (
				...
				in_array( $_REQUEST['action'], array( 'view', 'new', 'edit' ), true ) // phpcs:ignore WordPress.Security.NonceVerification
			) {
		...
		if ( self::PAGE_QUERY_BUILDER === $this->page ) {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				...
				wp_enqueue_script(
					'wpda_visual_query_builder',
					plugins_url( '../assets/premium/js/wpda_query_builder.js', __FILE__ ),
					array(),
					WPDA::get_option( WPDA::OPTION_WPDA_VERSION ),
					false
				);
		...
		if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
			...
			\WPDataAccess\Premium\WPDAPRO_FullText_Search\WPDAPRO_Search_Init::enqueue_scripts( $this->page );
			\WPDataAccess\Premium\WPDAPRO_Inline_Editing\WPDAPRO_Inline_Editing_Init::enqueue_scripts( $this->page );
	...
	public function add_menu_items() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( self::PAGE_MAIN === $this->page ) {
				$args                          = array( 'page_hook_suffix' => $this->wpda_data_explorer_menu );
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				// Add submenu for Chart.
				$this->wpda_charts_menu = add_submenu_page(
					$this->default_dashboard_menu,
					'WP Data Access',
					'Charts',
					'manage_options',
					self::PAGE_CHARTS,
					array( $this, 'data_charts_page' )
				);
				if ( self::PAGE_CHARTS === $this->page ) {
					$this->wpda_charts_view = new WPDA_List_View(
						array(
							'page_hook_suffix' => $this->wpda_charts_menu,
							'table_name'       => 'none',
							'list_table_class' => 'WPDataAccess\\Premium\\WPDAPRO_Charts\\WPDAPRO_Charts',
	...
	public function wpda_submenu_filter( $submenu_file ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			global $submenu;
			$submenu[ self::PAGE_MAIN ][0][2] = self::PAGE_DASHBOARD; // phpcs:ignore WordPress.WP.GlobalVariablesOverride
		}
	...
	public function data_explorer_page() {
		...
		if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
			if ( isset( $_REQUEST['page_action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				if ( 'geolocation_batch_update' === $_REQUEST['page_action'] ) { // phpcs:ignore WordPress.Security.NonceVerification
					WPDataAccess\Premium\WPDAPRO_Geo_Location\WPDAPRO_Geo_Location::start_geolocation_batch_update();
					return;
				}
			}
		}

		if ( isset( $_REQUEST['page_action'] ) && 'wpda_backup' === $_REQUEST['page_action'] ) { // phpcs:ignore WordPress.Security.NonceVerification
			$this->backup_page();
		} elseif ( isset( $_REQUEST['page_action'] ) && 'wpda_import_csv' === $_REQUEST['page_action'] ) { // phpcs:ignore WordPress.Security.NonceVerification
			$this->import_csv();
		} else {
	...
	public function data_designer_page_not_found() {
		...
		<div class="wrap">
			...
			<p>
				<?php echo __( 'ERROR: Repository table not found!', 'wp-data-access' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
			</p>
		</div>
		<?php
	}
