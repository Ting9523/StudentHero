...
namespace WPDataAccess\Data_Publisher {
	...
	class WPDA_Publisher_List_Table extends WPDA_List_Table {
		...
		public function get_columns() {
			...
			$columns = WPDA::array_insert_after(
				$columns,
				'pub_schema_name',
				array(
		...
		public function column_default( $item, $column_name ) {
			if ( 'pub_id' === $column_name ) {
				...
				if ( '' !== $warning ) {
					return $item['pub_id'] . $warning .
						substr( parent::column_default( $item, $column_name ), strlen( $item['pub_id'] ) );
				}
			}
			...
			if ( 'pub_data_source' === $column_name ) {
				switch( $item[ $column_name ] ) {
					case 'Query':
						...
						return '<div>' .
							'Custom query ' .
							'<i class="fas fa-eye wpda_tooltip_cq" title="' . $this->render_column_content( $item, 'pub_query', false ) . '"></i>' .
							'</div>';
					case 'CPT':
						// Show CPT query.
			...
			if (
				'pub_table_options_searching' === $column_name ||
				'pub_table_options_ordering' === $column_name ||
				'pub_table_options_paging' === $column_name ||
				'pub_table_options_serverside' === $column_name
			) {
...
EOT;
			...
			<script type='text/javascript'>
				jQuery("#wpda_invisible_container").append("<?php echo str_replace( "\n", '', $copy_form ); // phpcs:ignore WordPress.Security.EscapeOutput ?>");
			</script>
		...
		protected function get_unique_pub_name( $pub_id ) {
			...
			$db_pub_name = $wpdb->get_results(
				$wpdb->prepare(
					'select pub_name from `%1s` where pub_id = %d', // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
					array(
						WPDA::remove_backticks( WPDA_Publisher_Model::get_base_table_name() ),
						$pub_id,
					)
				),
				'ARRAY_A'
			...
			$wpdb->get_results(
				$wpdb->prepare(
					"select 'x' from `%1s` where pub_name = %s", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
					array(
						WPDA::remove_backticks( WPDA_Publisher_Model::get_base_table_name() ),
						$unique_pub_name,
					)
