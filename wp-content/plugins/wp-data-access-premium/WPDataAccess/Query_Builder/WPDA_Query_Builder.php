...
namespace WPDataAccess\Query_Builder {
	...
	class WPDA_Query_Builder {
		...
		const EXPLAIN_COMMANDS  = array( 'select', 'delete', 'insert', 'replace', 'update' );
		const EXPLAIN_PROTECTED = array( 'delete', 'insert', 'replace', 'update' );
			...
			?>
				<script type="application/javascript">
					...
					foreach ( $this->databases as $database ) {
						...
						echo "wpda_databases = '{$database_options}';"; // phpcs:ignore WordPress.Security.EscapeOutput
					?>
					var wpda_home_url = "<?php echo admin_url( 'admin.php' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>";
					var wpda_wpnonce = "<?php echo esc_attr( $this->wpnonce ); ?>";
					var wpda_loader_url = "<?php echo plugins_url( '../../assets/images/loading.gif', __FILE__ ); // phpcs:ignore WordPress.Security.EscapeOutput ?>";
					var vqbInstalled = false;
					<?php
					if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
						echo 'vqbInstalled = true;';
					}
					?>
		...
		public function save() {
			...
			if (
				...
				$wpda_sqlquery         = sanitize_textarea_field( wp_unslash( $_POST['wpda_sqlquery'] ) ); // input var okay.
				$wpda_vqb              = null;
				if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
					if ( isset( $_POST['wpda_vqb'] ) ) {
						$wpda_vqb = $_POST['wpda_vqb']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
					}
				}

				if ( wp_verify_nonce( $wpda_wpnonce, 'wpda-query-builder-' . WPDA::get_current_user_id() ) ) {
					// Save query
					if ( false === $this->upd_query( $wpda_schemaname, $wpda_sqlqueryname, $wpda_sqlquery, $wpda_sqlqueryname_old, $wpda_vqb ) ) {
		...
		protected function upd_query( $schema_name, $query_name, $query_sql, $query_name_old, $wpda_vqb = null ) {
			$wpda_query_builder_data = $this->get_query_list();
		...
		public function get_visual_query( $query_name ) {
			// Only implemented in premium version
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				$response = array(
					'content' => null,
					'status'  => 'OK',
				...
				if (
					...
					if ( wp_verify_nonce( $wpda_wpnonce, 'wpda-query-builder-' . WPDA::get_current_user_id() ) ) {
						$response['content'] = get_user_meta(
							WPDA::get_current_user_id(),
							'wpda_query_builder_' . $this->rewrite_query_name( $wpda_sqlqueryname ),
							true
						);
					} else {
						$response['status'] = 'Token expired, please refresh page';
		...
		protected function upd_visual_query( $query_name, $wpda_vqb ) {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				update_user_meta(
					WPDA::get_current_user_id(),
					'wpda_query_builder_' . $this->rewrite_query_name( $query_name ),
					$wpda_vqb
				);
			}
		}
