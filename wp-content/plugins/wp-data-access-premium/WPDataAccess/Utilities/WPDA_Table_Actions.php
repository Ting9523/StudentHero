...
namespace WPDataAccess\Utilities {
	...
	class WPDA_Table_Actions {
			...
			<script type='text/javascript'>
				function copyTable(rowNum, dboType, tableName, formId) {
					if (jQuery("#copy-table-from-" + rowNum).val()==="") {
						alert('<?php echo __( 'Please enter a valid table name', 'wp-data-access' ); ?>');
						return false;
					}

					if (confirm('<?php echo __( 'Copy', 'wp-data-access' ); ?> ' + dboType + '?')) {
		...
		public function tab_settings() {
			...
			$row_count_is_configurable =
				'innodb' === strtolower( $this->engine ) ||
				'federated' === strtolower( $this->engine ) ||
				'connect' === strtolower( $this->engine ) ||
				$this->engine === null;
			?>
			<style>
							...
							<ul style="margin:0; padding:0;">
								...
								<li>
									...
									<ul class="wpda_table_settings_nested">
										...
										<br/>
										<div style="font-size:90%;font-weight:bold;">
											<?php echo __( 'Query buffer size', 'wp-data-access' ); ?>
										</div>
										<div style="font-size:90%;">
											<label class="wpda_action_font">
												<?php echo __( 'Max rows per fetch', 'wp-data-access' ); ?>
											</label>
											<input type="text"
												   id="<?php echo esc_attr( $this->table_name ); ?>_query_buffer_size"
												<?php
...
Variable $$column_name$$ will be replaced with the value of column $$column_name$$ in table `' . esc_attr( $this->table_name ) . '`.', 'wp-data-access' ); ?>"
		...
		protected function tab_actions() {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				if (
					'CONNECT' === $this->engine &&
					\WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::is_pds_database( $this->schema_name )
				) {
					$is_pds = true;
				}
			}
			...
			<table class="widefat striped rows wpda-structure-table">
				...
				$this->tab_export();
				if (
					$this->is_wp_table === false &&
					( 'Table' === $this->dbo_type || 'View' === $this->dbo_type ) &&
					! $is_pds
				) {
					$this->tab_rename();
				}
		...
		protected function pds() {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				if (
					...
					$job = \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::stored_locally( $this->table_name, $this->schema_name );
					if ( false === $job ) {
						// Direct connections have no files to be synced.
						return;
					}

					$wp_nonce_action_refresh = "wpda-refresh-{$this->table_name}";
					$wp_nonce_refresh        = wp_create_nonce( $wp_nonce_action_refresh );
					$refresh_table_form_id   = 'refresh_table_form_' . esc_attr( $this->table_name );
					$refresh_table_form      =
						'<form' .
						" id='$refresh_table_form_id'" .
						" action='?page=" . esc_attr( \WP_Data_Access_Admin::PAGE_MAIN ) . "'" .
						" method='post'>" .
						"<input type='hidden' name='action' value='refresh-table' />" .
						"<input type='hidden' name='refresh_table_name' value='" . esc_attr( $this->table_name ) . "' />" .
						"<input type='hidden' name='_wpnonce' value='" . esc_attr( $wp_nonce_refresh ) . "' />" .
						'</form>';
					?>
					<tr>
						<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
							<script type='text/javascript'>
								jQuery("#wpda_invisible_container").append("<?php echo $refresh_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
							</script>
							<a href="javascript:void(0)"
							   class="button button-primary"
							   onclick="if (confirm('<?php echo __( 'Refresh table', 'wp-data-access' ) . ' ' . esc_attr( $this->table_name ) . '?'; ?>')) { jQuery('#<?php echo esc_attr( $refresh_table_form_id ); ?>').submit(); }"
							   style="display:block;"
							>
								<?php echo __( 'REFRESH', 'wp-data-access' ); ?>
							</a>
						</td>
							...
							<table style="border-spacing:0">
								<tr>
									<td style="padding:0 20px 0 0">
										...
										<br/>
										<?php echo __( 'Reloads remote data set and updates local table.', 'wp-data-access' ); ?>
									</td>
									<td style="padding:0 0 0 20px;border-left:1px solid #c3c4c7">
										<label style="width:130px;display:inline-block;">Remote file</label>
										<strong><?php echo $job['source']; ?></strong>
										<br/>
										<label style="width:130px;display:inline-block;">Next scheduled update:</label>
										<span class="wpda_pds_schedule">
											<span class="wpda_pds_schedule_info">
												<strong>
													<?php
													if ( null !== $job['exe_date'] && '' !== $job['exe_date'] ) {
														echo esc_attr( $job['exe_date'] ) . ' (interval ' . esc_attr( $job['interval'] ) . ' ' . esc_attr( $job['unit'] ) . 's)';
													}
													?>
												</strong>
												<a href="javascript:void(0)" onclick="jQuery(this).closest('.wpda_pds_schedule').find('span').toggle()" title="Change interval" class="wpda_tooltip">
													<i class="fas fa-edit"></i>
												</a>
											</span>
											<span class="wpda_pds_schedule_edit" style="display:none">
												<input type="number" class="pds_interval" style="width:80px" value="<?php echo esc_attr( $job['interval'] ); ?>">
												<select class="pds_interval_unit" style="vertical-align:baseline">
													<option value=""></option>
													<option value="hour" <?php echo 'hour' === $job['unit'] ? 'selected' : ''; ?>>hours</option>
													<option value="day" <?php echo 'day' === $job['unit'] ? 'selected' : ''; ?>>days</option>
												</select>
												<a href="javascript:void(0)" class="button button-secondary wpda_tooltip" onclick="updatePdsInterval(jQuery(this), '<?php echo esc_attr( $this->schema_name ); ?>', '<?php echo esc_attr( $this->table_name ); ?>')" title="Reschedule next update and change interval">OK</a>
												<label>
...
Leave empty if updates are not required." style="cursor:pointer"></span>
												...
												<a href="javascript:void(0)" onclick="jQuery(this).closest('.wpda_pds_schedule').find('span').toggle()" title="Close" class="wpda_tooltip">
													<i class="fas fa-times-circle wpda_icon_on_button"></i>
												</a>
											</span>
										</span>
		...
		protected function tab_copy() {
			...
			$copy_table_form      =
				...
				"<input type='hidden' name='copy_schema_name_src' value='" . esc_attr( $this->schema_name ) . "' />" .
				"<input type='hidden' name='copy_table_name_src' value='" . esc_attr( $this->table_name ) . "' />" .
				"<input type='hidden' name='copy_schema_name_dst' id='copy_schema_name_" . esc_attr( $this->table_name ) . "' value='' />" .
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					...
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="copyTable('<?php echo esc_attr( $this->rownum ); ?>', '<?php echo esc_attr( $this->dbo_type ); ?>', '<?php echo esc_attr( $this->table_name ); ?>', '<?php echo esc_attr( $copy_table_form_id ); ?>')"
					   style="display:block;"
					>
				...
				<td style="vertical-align:middle;">
					...
					<select id="copy-schema-from-<?php echo esc_attr( $this->rownum ); ?>">
						<?php
						...
						foreach ( $schema_names as $schema_name ) {
							...
							echo
								'<option value="' . esc_attr( $schema_name['schema_name'] ) . '" ' . $selected . '>' .
									esc_attr( $database ) .
								'</option>';
						}
						?>
