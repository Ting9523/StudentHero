	...
	class WPDA_Settings {
		...
		public function __construct( $current_tab, $help_url ) {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				// Add dashboard tab
				$this->tabs =
					array_slice( $this->tabs, 0, 3, true ) +
					array(
						'dashboard' => 'Dashboard',
					) +
					array_slice( $this->tabs, 3, count( $this->tabs ) - 1, true );

				// Add Data Forms tab
				$this->tabs =
					array_slice( $this->tabs, 0, 4, true ) +
					array(
						'dataforms' => 'Data Forms',
					) +
					array_slice( $this->tabs, 4, count( $this->tabs ) - 1, true );
			...
			<div class="wrap">
				<h1>
					...
					if ( WPDA_Dashboard::add_actions_to_page_title() ) {
						?>
						<a href="<?php echo $this->help_url; // phpcs:ignore WordPress.Security.EscapeOutput ?>" target="_blank" class="wpda_tooltip" title="Plugin Help - open a new tab or window">
