...
namespace WPDataAccess\Simple_Form {
	...
	class WPDA_Simple_Form {
		...
		public function __construct(
			...
			if (
				isset( $wpda_project_mode['allow_insert'] ) &&
				'only' === $wpda_project_mode['allow_insert']
			) {
		...
		protected function get_url_arguments() {
			...
			foreach ( $this->wpda_list_columns->get_table_columns() as $column ) {
				if ( isset( $_REQUEST[ $column['column_name'] . '_old' ] ) ) {
					$this->form_items_old_values[ $column['column_name'] ] =
						wp_unslash( $_REQUEST[ $column['column_name'] . '_old' ] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
				}
				if ( isset( $_REQUEST[ $column['column_name'] ] ) ) {
					if ( is_array( $_REQUEST[ $column['column_name'] ] ) ) {
						$column_array = '';
						foreach ( $_REQUEST[ $column['column_name'] ] as $column_value ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
						...
						$this->form_items_new_values[ $column['column_name'] ] =
							wp_unslash( $_REQUEST[ $column['column_name'] ] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
					}
				}
			}
		}
		...
		public function show( $allow_save = true, $add_param = '' ) {
			...
			<div class="wrap">
				<?php if ( $this->show_title ) { ?>
					<h1>
						...
						<a
								...
								title="<?php echo esc_attr( $this->back_to_list_text ); ?>"
						></a>
						<?php } ?>
						...
						if (
							...
							if ( null !== $this->help_url ) {
								?>
								<a href="<?php echo esc_attr( $this->help_url ); ?>" target="_blank" class="wpda_tooltip" title="Plugin Help - open a new tab or window">
					...
					<div class="wpda_subtitle"><strong><?php echo wp_kses( $this->subtitle, array( 'span' => array( 'class' => array() ) ) ); ?></strong>
					</div>
					<?php
				...
				<form id="<?php echo esc_attr( $this->current_form_id ); ?>"
						...
						foreach ( $this->fieldsets as $fieldset_title => $fieldset ) {
							...
							<fieldset class="wpda_fieldset">
								<?php if ( '' !== $fieldset_title ) { ?>
									<legend <?php echo '' === $id ? '' : 'id="' . esc_attr( $id ) . '"'; ?>>
										<?php
										if ( $expandable && '' !== $id ) {
											echo '<div class="wpda-fieldset-expand" onclick="expandFieldset(\'' . esc_attr( $id ) . '\')">' .
													'<i id="wpda_field_expand_' . esc_attr( $id ) . '" class="wpda-fieldset-expand fas fa-' . esc_attr( $expandable_icons) . '"></i> ' .
													'<span class="wpda-fieldset-expand">' . esc_html( $fieldset_title ) . '</span>' .
													'<input type="hidden" id="wpda_fieldset_expand_' . esc_attr( $id ) . '" name="wpda_fieldset_expand_' . esc_attr( $id ) . '" value="' . esc_attr( $expandable_state ) . '" />' .
												'</div>';
										} else {
								...
								<?php } ?>
								<table id="wpda_table_expand_<?php echo esc_attr( $id ); ?>"
									   class="wpda_simple_table <?php echo esc_attr( $this->table_name ); ?>"
									   <?php echo $expandable && 'off' === $expandable_state ? 'style="display: none"' : ''; ?>
									   cellspacing="0"
									   cellpadding="0"
								>
									<?php
					...
					echo $this->page_number_item; // phpcs:ignore WordPress.Security.EscapeOutput
					...
					<?php if ( 'view' !== $this->action ) { ?>
						...
						if ( $this->show_back_button && $this->show_back_icon ) {
							...
							<button type="submit"
								...
								<?php echo esc_attr( $this->back_to_list_text ); ?>
							</button>
							<?php
						}
			...
			<script type='text/javascript'>
				<?php if ( 'view' !== $this->action ) { ?>
					function submit_form(e) {
						...
						if (
							...
							<?php if ( 'child_page_number' === $this->page_number_item_name ) { ?>
				...
				<?php echo $js_code; // phpcs:ignore WordPress.Security.EscapeOutput ?>
			</script>
			<?php
		}
