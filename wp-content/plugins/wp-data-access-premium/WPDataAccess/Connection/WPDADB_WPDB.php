...
namespace WPDataAccess\Connection {
	...
	class WPDADB_WPDB extends \wpdb {
		...
		public function __construct(
			$dbuser, $dbpassword, $dbname, $dbhost, // Original WordPress wpdb parameters
			$ssl = 'OFF',
			$ssl_key = null,
			$ssl_cert = null,
			$ssl_ca = null,
			...
			if ( $this->use_mysqli ) {
				...
				if ( 'on' === $this->ssl ) {
					...
					if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
						if ( \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::is_pds_server( $this->dbhost ) ) {
							mysqli_ssl_set( // phpcs:ignore
								$this->dbh,
								null,
								null,
								plugin_dir_path( __DIR__ ) . 'Premium/WPDAPRO_Data_Services/', // This works even without a CRT file :-)
								null,
								null
							);
						}
