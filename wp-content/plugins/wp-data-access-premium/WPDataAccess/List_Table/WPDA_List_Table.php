...
namespace WPDataAccess\List_Table {
	...
	class WPDA_List_Table extends Wordpress_Original\WP_List_Table {
		...
		public function column_default( $item, $column_name ) {
			if (
					$this->wpda_list_columns->get_table_columns()[0]['column_name'] === $column_name ||
				...
				if ( 0 === $count ) {
					...
					return $this->render_column_content( $item, $column_name ) . $this->row_actions( array( 'wpda_hide_column' => '' ) );
				} else {
					// Check rights.
					...
					if ( 'on' === $this->show_view_link ) {
						...
						<script type='text/javascript'>
							jQuery("#wpda_invisible_container").append("<?php echo $view_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
						</script>
					...
					if ( 'on' === $this->allow_update || WPDA::is_wpda_table( $this->table_name ) ) {
						...
						<script type='text/javascript'>
							jQuery("#wpda_invisible_container").append("<?php echo $edit_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
						</script>
					...
					if ( 'on' === $this->allow_delete || WPDA::is_wpda_table( $this->table_name ) ) {
						...
						<script type='text/javascript'>
							jQuery("#wpda_invisible_container").append("<?php echo $delete_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
						</script>
		 ...
		 */
		private function create_post_form( $item, $form_id, $action, $url, $wp_nonce, $row_security_nonce_field, $schema_name, $table_name ) {
...
EOT;
			...
			if ( 'off' === WPDA::get_option( WPDA::OPTION_BE_TEXT_WRAP_SWITCH ) &&
				...
				if ( $substitute_newlines ) {
					return str_replace(
						"\n",
						'<br/>',
						substr( esc_html( str_replace( '&', '&amp;', $column_content ) ), 0, WPDA::get_option( WPDA::OPTION_BE_TEXT_WRAP ) ) .
		...
		protected function show_sub_title() {
			?>
			<div class="wpda_subtitle"><strong><?php echo wp_kses( $this->subtitle, array( 'span' => array( 'class' => array() ) ) ); ?></strong></div>
			<?php
		}
		...
		public function show() {
			...
			<div class="wrap<?php echo is_admin() ? '' : ' wp-core-ui'; ?>">
				...
				<h1 class="wp-heading-inline">
					...
					if (
						...
						<a
								...
								title="<?php echo \WP_Data_Access_Admin::PAGE_MAIN === $this->page ? 'Data Explorer' : __( 'Favourites', 'wp-data-access' ); ?>"
						></a>
						<?php
					}
					...
					if (
						WPDA_Dashboard::add_actions_to_page_title() &&
						'' !== $help_url
					) {
						?>
						&nbsp;
						<span>
							<a href="<?php echo esc_url( $help_url ); ?>" target="_blank"
				...
				if (
						! is_admin() ||
						'wpda_wpdp_' === substr( $this->page, 0, 10 ) ||
						WPDP::PAGE_TEMPLATES === $this->page ||
						WPDA_Dashboard::add_actions_to_page_title() ||
						substr( $this->page, 0, 13 ) === \WP_Data_Access_Admin::PAGE_EXPLORER
				) {
				...
				<div id="wpda_invisible_container" style="display:none;">
					<form id="wpda_main_export_form" method="post" action="<?php echo esc_url( $action_url ); ?>?action=wpda_export" target="_blank">
					...
					<form id="wpda_table_export_form" method="post" action="<?php echo esc_url( $action_url ); ?>?action=wpda_export" target="_blank">
					...
					<form id="wpda_row_export_form" method="post" action="<?php echo esc_url( $action_url ); ?>?action=wpda_export" target="_blank">
				...
				>
					...
					if ( self::LIST_BASE_TABLE !== $this->table_name ) {
						...
						<input type="hidden" name="wpdaschema_name"
							   value="<?php echo esc_attr( $this->schema_name ); // input var okay. ?>"/>
						<input type="hidden" name="table_name"
							   value="<?php echo esc_attr( $this->table_name ); // input var okay. ?>"/>
						<?php
					}
					?>
					...
					<?php wp_nonce_field( 'wpda-export-' . wp_json_encode( $this->table_name ), '_wpnonce', false ); ?>
					<?php wp_nonce_field( "wpda-delete-{$this->table_name}", '_wpnonce2', false ); ?>
					<?php
					if ( self::LIST_BASE_TABLE === $this->table_name ) {
			...
			<script type='text/javascript'>
				jQuery(function() {
					...
					jQuery("#wpda_toolbar_icon_add_row").on("click", function() {
						jQuery("#wpda_new_row_table_name").val("<?php echo esc_attr( $this->table_name ); ?>");
						jQuery("#wpda_new_row").submit();
					});
					jQuery("#wpda_toolbar_icon_add_publication").on("click", function() {
						jQuery("#wpda_new_publication_table_name").val("<?php echo esc_attr( $this->table_name ); ?>");
					...
					>
						<div>
							...
							<button type="submit" class="page-title-action wpda_tooltip"
									title="<?php echo sprintf( __( 'Add new %1$s to %2$s', 'wp-data-access' ), esc_attr( $this->_args['singular'] ), esc_attr( $storage_type ) ); ?>"
							>
		...
		public function process_bulk_action() {
			...
			switch ( $this->current_action() ) {
				case 'delete':
					...
					$row_to_be_deleted = array(); // Gonna hold the row to be deleted.
				...
				case 'bulk-export-csv':
					...
					<script type="application/javascript">
						jQuery(function() {
							wpda_row_export(
								'<?php echo esc_attr( $this->pid ); ?>',
								'<?php echo esc_attr( $submit_schema_name ); ?>',
								'<?php echo esc_attr( $submit_table_name ); ?>',
								'<?php echo esc_attr( $wp_nonce ); ?>',
								'<?php echo esc_attr( $format_type ); ?>',
								'off',
								'off',
								'off',
								'<?php echo wp_json_encode( $columns ); ?>'
							);
						});
					</script>
					<?php
		...
		public function delete_row( $where, $engine = '' ) {
			...
			if ( 'connect' === strtolower( $engine ) ) {
				if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
					// Optimize table: needed for indexed remote data files
					if ( \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::is_pds_database( $this->schema_name ) ) {
						\WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::optimize_table( $wpdadb, $this->table_name );
					}
				}
		...
		public function record_count() {
			...
			return $wpdadb->get_var( $query ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
		}
		...
		public function get_rows() {
			...
			$this->items = $wpdadb->get_results( $query, 'ARRAY_A' ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
		}
		...
		protected function get_order_by() {
			if ( ! empty( $_REQUEST['orderby'] ) ) {
				$orderby_arg = sanitize_sql_orderby( wp_unslash( $_REQUEST['orderby'] ) ); // input var okay.
		...
		protected function get_search_value() {
			if ( 'off' === WPDA::get_option( WPDA::OPTION_BE_REMEMBER_SEARCH ) ) {
				if ( isset( $_REQUEST[ $this->search_item_name ] ) ) {
					return wp_filter_nohtml_kses( wp_unslash( $_REQUEST[ $this->search_item_name ] ) ); // input var okay.
				}
			}
			...
			if ( 'wpda_wpdp_' === substr( $this->page, 0, 10 ) ) {
				...
				if ( isset( $_REQUEST['child_request'] ) ) {
					$cookie_name = 'NOCOOKIESFORCHILDREQUESTS'; // No search values stored for child tables.
				}
			...
			if ( isset( $_REQUEST[ $this->search_item_name ] ) && '' !== $_REQUEST[ $this->search_item_name ] ) { // input var okay.
				...
				return wp_filter_nohtml_kses( wp_unslash( $_COOKIE[ $cookie_name ] ) ); // input var okay.
			} else {
				return null;
			}
		}
		...
		public function print_column_headers( $with_id = true ) {
			...
			if ( isset( $_REQUEST['order'] ) && 'desc' === $_REQUEST['order'] ) { // input var okay.
				$current_order = 'desc';
			} else {
				$current_order = 'asc';
			}
		...
		protected function add_full_table_downloads() {
			// This feature is only implemented in the premium version.
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				$show_download_buttons = false;
				if ( is_admin() ) {
					// Buttons are only added on back-end (user needs admin access).
					if ( self::LIST_BASE_TABLE === $this->table_name ) {
						$show_download_buttons = false;
					} else {
						if ( current_user_can( 'manage_options' ) ) {
							...
							global $wpda_project_mode;
							if ( isset( $wpda_project_mode['page_allow_full_export'] ) ) {
								$show_download_buttons = 'yes' === $wpda_project_mode['page_allow_full_export'];
							}
						}
					}
				}
				if ( $show_download_buttons ) {
					$wp_nonce_action = 'wpda-export-' . wp_json_encode( $this->table_name );
					$wp_nonce        = wp_create_nonce( $wp_nonce_action );
					$admin_url       = admin_url( 'admin.php' );
					?>
					<span>
						<a class="button button-secondary wpda_full_table_download wpda_tooltip"
							onclick="download_csv()"
							title="Start full table download to CSV"
							target="_blank"
						>
						...
						<a class="button button-secondary wpda_full_table_download wpda_tooltip"
							...
							title="Start full table download to JSON"
							target="_blank"
						>
							<i class="fas fa-cloud-download wpda_icon_on_button"></i>
							JSON
					...
					<script type='text/javascript'>
						function download_csv() {
							jQuery("#wpda_download_csv").submit();
							return true;
						}
						function download_json() {
							jQuery("#wpda_download_json").submit();
							return false;
						}
						jQuery(function() {
							if (jQuery("#wpda_download_csv").length===0) {
								// Prevent double addition (header + footer)
								jQuery("#wpda_invisible_container").append(`
									<form id='wpda_download_csv' method='post' action='<?php echo esc_url( $admin_url ); ?>'>
										<input type='hidden' name='action' value='wpda_export'/>
										<input type='hidden' name='wpdaschema_name' value='<?php echo esc_attr( $this->schema_name ); ?>'/>
										<input type='hidden' name='table_names' value='<?php echo esc_attr( $this->table_name ); ?>'/>
								...
								`);
								jQuery("#wpda_invisible_container").append(`
									<form id='wpda_download_json' method='post' action='<?php echo esc_url( $admin_url ); ?>'>
										<input type='hidden' name='action' value='wpda_export'/>
										<input type='hidden' name='wpdaschema_name' value='<?php echo esc_attr( $this->schema_name ); ?>'/>
										<input type='hidden' name='table_names' value='<?php echo esc_attr( $this->table_name ); ?>'/>
										<input type='hidden' name='type' value='table'/>
