...
namespace WPDataAccess\List_Table {
	...
	class WPDA_List_Table_Menu extends WPDA_List_Table {
		...
		public function __construct( $args = array() ) {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				if ( isset( $_REQUEST['action'] ) && 'pds' === $_REQUEST['action'] ) {
					$pds = new \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services();
					$pds->upload_post( $wpdadb, $this->schema_name );
				}
			}
		...
		public function single_row( $item ) {
			...
			if ( $this->bulk_actions_enabled ) {
				echo '<tr style="height:0 !important; padding: 0 !important;"><td id="rownum_' . esc_attr( self::$list_number ) . '_x1" colspan="' . esc_attr( 13 - $column_not_showns ) . '" style="padding:0 !important;"></tr>'; // Fake! Maintain odd/even colors.
				...
				echo '<tr style="height:0 !important; padding: 0 !important;"><td id="rownum_' . esc_attr( self::$list_number ) . '_x1" colspan="' . esc_attr( 12 - $column_not_showns ) . '" style="padding:0 !important;"></tr>'; // Fake! Maintain odd/even colors.
		...
		public function column_default( $item, $column_name ) {
			...
			if ( 'table_rows' === $column_name ) {
				if (
					'federated' === strtolower( $item['engine'] ) ||
					'connect' === strtolower( $item['engine'] ) ||
					'innodb' === strtolower( $item['engine'] ) ||
					stripos( strtolower( $item['table_type'] ), 'view' ) !== false
				) {
			...
			if ( 'table_name' === $column_name ) {
				...
				$admin_actions = array(); // Array containing admin actions.
				...
				if ( WPDA::can_manage() ) {
					...
					$actions['manage']             =
						...
						'<i class="fas fa-gears wpda_icon_on_button"></i> ' . __( 'Manage', 'wp-data-access' ) .
						'</a>';
				}
				...
				if ( 'on' === WPDA::get_option( WPDA::OPTION_BE_CONFIRM_VIEW ) ) {
					if ( 'VIEW' !== strtoupper( $item['table_type_db'] ) && 'SYSTEM VIEW' !== strtoupper( $item['table_type_db'] ) ) {
						if ( strtolower( WPDA::TABLE_TYPE_WP ) === strtolower( $item['table_type'] ) ) {
							// WordPress table.
							$msg = __(
								'You are about to edit a WordPress table! Changing this table might result in corrupting the WordPress database. Are you sure you want to continue?',
								'wp-data-access'
							);
						} elseif ( strtolower( WPDA::TABLE_TYPE_WPDA ) === strtolower( $item['table_type'] ) ) {
							// Plugin table.
							$msg = __(
								'You are about to edit a plugin table! Changing this table might result in corrupting the WP Data Access database. Are you sure you want to continue?',
								'wp-data-access'
							);
						} else {
							// User table (other than WordPress or plugin).
							$msg = __(
								'You are about to edit a table of an external application! Changing this table might result in corrupting the external database. Are you sure you want to continue?',
								'wp-data-access'
							);
						}
...
EOT;
				...
				<script type='text/javascript'>
					jQuery("#wpda_invisible_container").append("<?php echo $explore; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
				</script>

				<?php
				$action_view          =
					' <i class="fas fa-table-list wpda_icon_on_button"></i> ' .
				...
				if (
					strtolower( WPDA::TABLE_TYPE_WP ) !== strtolower( $item['table_type'] )
					&& strtolower( WPDA::TABLE_TYPE_WPDA ) !== strtolower( $item['table_type'] )
				) {
		...
		protected function count_rows( $table_name ) {
			...
			$count = @$wpdadb->get_var( $query ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
			if ( ! is_numeric( $count ) ) {
		...
		protected function process_bulk_action_refresh_table() {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				$table_name = isset( $_REQUEST['refresh_table_name'] ) ?
					sanitize_text_field( wp_unslash( $_REQUEST['refresh_table_name'] ) ) : '';
				if ( ! $this->process_bulk_action_check_wpnonce( 'wpda-refresh-' . esc_attr( $table_name ), '_wpnonce' ) ) {
					return;
				}

				if ( false === \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::refresh_table( $table_name, $this->schema_name ) ) {
					$msg = new WPDA_Message_Box(
						array(
							'message_text'           => sprintf( __( 'Table refresh failed', 'wp-data-access' ) ) . " [{$table_name}]",
							'message_type'           => 'error',
					...
					$msg = new WPDA_Message_Box(
						array(
							'message_text' => sprintf( __( 'Table `%s` updated', 'wp-data-access' ), $table_name ),
						)
					);
					$msg->box();
				}
		...
		protected function process_bulk_action_drop_db() {
			...
			if ( 'rdb:' === substr( $database, 0, 4 ) ) {
				...
				if (
					false === $wpdb->query(
							$wpdb->prepare(
								'drop database `%1s`', // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
		...
		protected function process_bulk_action_create_db() {
			...
			if ( isset( $_REQUEST['database_location'] ) && 'local' === $_REQUEST['database_location'] ) {
				...
				if (
					false === $wpdb->query(
						$wpdb->prepare(
							'create database `%1s`', // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
							array(
								WPDA::remove_backticks( $database ),
							)
						)
					)
				) { // db call ok; no-cache ok.
		...
		protected function process_bulk_action_settings_table() {
			if ( isset( $_REQUEST['settings_table_name'] ) && isset( $_REQUEST['settings'] ) ) {
				...
				$settings_object     = json_decode( WPDA::sanitize_text_field_array( $_REQUEST['settings'] ) );  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
		...
		protected function process_bulk_action_copy_table() {
			...
			if ( $this->process_bulk_action_check_action(
				...
				$copy_schema_name_src = sanitize_text_field( wp_unslash( $_REQUEST['copy_schema_name_src'] ) ); // input var okay.
				$copy_table_name_src  = sanitize_text_field( wp_unslash( $_REQUEST['copy_table_name_src'] ) ); // input var okay.
				$copy_schema_name_dst = sanitize_text_field( wp_unslash( $_REQUEST['copy_schema_name_dst'] ) ); // input var okay.
							...
							if (
								...
								$msg = new WPDA_Message_Box(
									array(
										'message_text' =>
											strtoupper( substr( $dbo_type, 0, 1 ) ) . strtolower( substr( $dbo_type, 1 ) ) .
											" `{$copy_schema_name_src}`.`{$copy_table_name_src}` " . __( 'copied to', 'wp-data-access' ) .
											" `{$copy_schema_name_dst}`.`{$copy_table_name_dst}` ",
									)
								);
								$msg->box();
							}
		...
		protected function copy_table(
			...
			$ctcmd = $wpdadb_src->get_results( $query, 'ARRAY_A' ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
			...
			if ( $copy_table_name_src !== $copy_table_name_dst ) {
				...
				if ($pos !== false) {
					$create_table_statement = substr_replace( $create_table_statement, $copy_table_name_dst, $pos, strlen( $copy_table_name_src ) );
				}
			}
			...
			if ( is_numeric( $query_buffer_size ) && $query_buffer_size > 0 ) {
				...
				<p class="wpda_pds_msg_create">
					Copy table `<?php echo esc_attr( $copy_schema_name_src ); ?>`.`<?php echo esc_attr( $copy_table_name_src ); ?>`
				</p>
				<p class="wpda_pds_msg_create">
					To `<?php echo esc_attr( $copy_schema_name_dst ); ?>`.`<?php echo esc_attr( $copy_table_name_dst ); ?>`
				</p>
				<p class="wpda_pds_msg_create">
					Hang on...
				</p>
				<p class="wpda_pds_msg_create">
					Table `<?php echo esc_attr( $copy_table_name_dst ); ?>` created...
				</p>
				<p class="wpda_pds_msg_create">
					Copying data...
				</p>
				...
				$rows  = $wpdadb_src->get_results( $sql, 'ARRAY_A' ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
				...
				$rows = $wpdadb_src->get_results( $query, 'ARRAY_A' ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
				foreach ( $rows as $row ) {
					$wpdadb_dst->insert(
						$copy_table_name_dst,
						$row
						...
						<script type="application/javascript">
							jQuery(function() {
								wpda_main_export(
									...
									'<?php echo json_encode( $selected_tables ); ?>'
								);
							});
						</script>
						<?php
		...
		protected function post_drop_table( $drop_table_name ) {
			...
			$wpdb->query(
				$wpdb->prepare(
					'delete from `%1s` where wpda_schema_name = %s and wpda_table_name = %s ', // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
					array(
						WPDA::remove_backticks( WPDA_Table_Settings_Model::get_base_table_name() ),
						$this->schema_name,
						$drop_table_name,
					)
				)
			...
			$wpdb->query(
				$wpdb->prepare(
					'delete from `%1s` where media_schema_name = %s and media_table_name = %s ', // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
					array(
						WPDA::remove_backticks( WPDA_Media_Model::get_base_table_name() ),
						$this->schema_name,
						$drop_table_name,
					)
				)
			...
			$wpdb->query(
				$wpdb->prepare(
					'delete from `%1s` where menu_schema_name = %s and menu_table_name = %s ', // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders
					array(
						WPDA::remove_backticks( WPDA_User_Menus_Model::get_base_table_name() ),
						$this->schema_name,
						$drop_table_name,
					)
				)
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				if ( \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::is_pds_database( $this->schema_name ) ) {
					// Some premium data services are file based. Delete files after drop table.
					$wpdadb = WPDADB::get_db_connection( $this->schema_name );
					if ( null !== $wpdadb ) {
						\WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::drop_table( $wpdadb, $drop_table_name, $this->schema_name );
					}
				}
			}
		...
		protected function add_header_actions() {
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				if ( \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services::is_pds_database( $this->schema_name ) ) {
					$pds = new \WPDataAccess\Premium\WPDAPRO_Data_Services\WPDAPRO_Data_Services();
					$pds->upload_form();
					$pds->message_box( $this->schema_name );
					$pds->add_ajax();
				}
			}
		...
		public function show() {
			...
			<script type='text/javascript'>
				function show_hide_column(show) {
					for (i = 0; i <<?php echo esc_attr( self::$list_number ); ?>; i++) {
						if (show) {
		...
		protected function add_header_button() {
			...
			<form id="wpda_linkto_backup" style="display: none" method="post" action="?page=<?php echo esc_attr( WP_Data_Access_Admin::PAGE_MAIN ); ?>&page_action=wpda_backup">
			...
			><i class="fas fa-cloud-download wpda_icon_on_button"></i> <?php echo __( 'Data Backup' ); ?></a>
			<?php
		}
		...
		protected function add_db_containers() {
			...
			<div id="wpda_db_edit" style="display:none;padding-top:10px;">
				<div class="wpda_upload">
					<form method="post" action="?page=<?php echo esc_attr( $this->page ); ?>" onsubmit="return editdb_validate_form();">
						...
						<div>
							...
							<input type="text" name="edit_remote_database" id="edit_remote_database" value="<?php echo esc_attr( $this->schema_name ); ?>" <?php echo esc_attr( $readonly ); ?>>
							...
							<input type="password" name="edit_remote_passwd" id="edit_remote_passwd" value="<?php echo esc_attr( $passwd ); ?>" autocomplete="new-password">
							<br/>
						...
						<input type="submit" class="button button-prrimary" value="<?php echo __( 'Save', 'wp-data-access' ); ?>">
						<a href="javascript:void(0)"
						   onclick="jQuery('#wpda_db_edit').hide()"
						   class="button button-secondary"><i class="fas fa-times-circle wpda_icon_on_button"></i> <?php echo __( 'Cancel', 'wp-data-access' ); ?></a>
						<input type="hidden" name="action" value="edit_db">
						<?php wp_nonce_field( 'wpda-edit-db-from-data-explorer-' . WPDA::get_current_user_login(), '_wpnonceeditdb', false ); ?>
					</form>
				</div>
			</div>
			...
			<div id="wpda_db_container" style="display:none;padding-top:10px;">
				<div class="wpda_upload">
					<form method="post" action="?page=<?php echo esc_attr( $this->page ); ?>" onsubmit="return createdb_validate_form();">
						<div style="height:10px;"></div>
						...
						<a href="javascript:void(0)"
						   ...
						   class="button button-primary"><i class="fas fa-cloud-upload wpda_icon_on_button"></i> <?php echo __( 'Save', 'wp-data-access' ); ?></a>
						<a href="javascript:void(0)"
						   onclick="jQuery('#wpda_db_container').hide()"
						   class="button button-secondary"><i class="fas fa-times-circle wpda_icon_on_button"></i> <?php echo __( 'Cancel', 'wp-data-access' ); ?></a>
						<input type="hidden" name="action" value="create_db">
						<?php wp_nonce_field( 'wpda-create-db-from-data-explorer-' . WPDA::get_current_user_login(), '_wpnoncecreatedb', false ); ?>
					</form>
				</div>
			</div>

			<div style="display:none;">
				<form id="wpda_form_drop_db" method="post" action="?page=<?php echo esc_attr( $this->page ); ?>">
					<input type="text" name="database" id="drop_database">
					<input type="hidden" name="action" value="drop_db">
					<?php wp_nonce_field( 'wpda-drop-db-from-data-explorer-' . WPDA::get_current_user_login(), '_wpnoncedropdb', false ); ?>
				</form>
			</div>
		...
		public function search_box( $text, $input_id ) {
			...
			<div style="padding-top:10px;padding-bottom:0;">
				...
				<select id="wpda_main_db_schema_list">
					...
					foreach ( $schemas as $schema ) {
						...
						<option value="<?php echo esc_attr( $schema['schema_name'] ); ?>" <?php echo esc_attr( $selected ); ?>>
							<?php echo esc_attr( $printed_schema_name ); ?>
						</option>
						<?php
					}
					?>
				...
				<a class="dashicons dashicons-plus-alt wpda_tooltip" href="javascript:void(0)"
				   ...
				   title="<?php echo esc_attr( $this->user_create_db_hint ); ?>">&nbsp;</a>
				<a class="dashicons dashicons-dismiss wpda_tooltip" id="wpda_drop_database" href="javascript:void(0)"
					...
					if ( $this->user_can_drop_db ) {
						...
						?>
						onclick="if (confirm('<?php echo $msg; // phpcs:ignore WordPress.Security.EscapeOutput ?>')) { jQuery('#drop_database').val(jQuery('#wpda_main_db_schema_list').val()); jQuery('#wpda_form_drop_db').submit(); }"
						<?php
					}
					?>
				   style="<?php echo $this->user_can_drop_db ? 'color:#a00;' : 'color:grey;cursor:default;'; ?>vertical-align:middle;"
				   title="<?php echo esc_attr( $this->user_drop_db_hint ); ?>">&nbsp;</a>
				<a class="dashicons dashicons-admin-plugins wpda_tooltip"
				   href="javascript:void(0)"
				   onclick="wpda_dbinit_admin( '<?php echo esc_attr( $this->schema_name ); ?>', '<?php echo wp_create_nonce( 'wpda_dbinit_admin_' . WPDA::get_current_user_login() ); ?>' )"
				   style="vertical-align:middle;"
				   title="<?php echo __( "Create function wpda_get_wp_user_id()\nto get access to the WordPress user ID from database views", 'wp-data-access' ); ?>">&nbsp;</a>
				&nbsp;<span style="font-weight: bold;"><?php echo __( 'Favourites', 'wp-data-access' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>:</span>
