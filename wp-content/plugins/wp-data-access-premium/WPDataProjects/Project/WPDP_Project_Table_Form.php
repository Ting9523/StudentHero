	...
	class WPDP_Project_Table_Form {
		...
		protected function show_title() {
			...
			<h1 class="wp-heading-inline">
				<a
					...
					title="<?php echo __( 'Template list', 'wp-data-access' ); ?>"
				></a>
				...
				if ( WPDA_Dashboard::add_actions_to_page_title() ) {
					...
					?>
					<span>
						<a href="<?php echo esc_attr( $help_url ); ?>" target="_blank"
		...
		protected function show_table_info() {
			...
			$hyperlinks_parent = isset( $this->table_structure->tableinfo->hyperlinks_parent ) ?
				$this->table_structure->tableinfo->hyperlinks_parent : array();
			$hyperlinks_child  = isset( $this->table_structure->tableinfo->hyperlinks_child ) ?
				$this->table_structure->tableinfo->hyperlinks_child : array();
			...
			<form id="wpdp_form_table_info" method="post">
				...
				<fieldset class="wpda_fieldset wpdp_fieldset">
					...
					<table class="wpda-table-structure wpdp-table-structure">
						...
						<tr>
							...
							<td>
								<?php echo $html_child; // phpcs:ignore WordPress.Security.EscapeOutput ?>
							</td>
						</tr>
					</table>
				</fieldset>
				...
				<div class="wpdp-button-panel">
					...
					<a href="?page=<?php echo esc_attr( $this->page ); ?>" class="button button-secondary">
		...
		protected function show_relations() {
			...
			<script type='text/javascript'>
				...
				function wpdp_get_columns(table_name, index, target_id, selected_value, schema_name = '') {
					if (schema_name === '') {
						schema_name = '<?php echo esc_attr( $this->wpda_schema_name ); ?>';
					}
				...
				function add_row(relation_type, source_column_name, target_table_name, target_column_name, relation_table_name, target_schema_name = '') {
					...
					if (target_schema_name === '' || target_schema_name === '<?php echo esc_attr( $this->wpda_schema_name ); ?>') {
						target_schema_name = '<?php echo esc_attr( $this->wpda_schema_name ); ?>';
					...
					var new_row = '<tr id="relation_' + row_num + '">' +
						...
						foreach ( $source_table_columns as $column ) {
							?>
							'<option value="<?php echo esc_attr( $column['column_name'] ); ?>" ' + (source_column_name[0] === "<?php echo esc_attr( $column['column_name'] ); ?>" ? "selected" : "") + '><?php echo esc_attr( $column['column_name'] ); ?></option>' +
							<?php
						}
						?>
						'    </select>' +
						...
						foreach ( $available_databases as $available_database ) {
							...
							?>
						'<option value="<?php echo esc_attr( $available_database['schema_name'] ); ?>" ' + (target_schema_name === "<?php echo esc_attr( $available_database['schema_name'] ); ?>" ? "selected" : "") + '><?php echo esc_attr( $database_printed ); ?></option>' +
							<?php
						}
						?>
						'    </select>' +
					...
					foreach ( $target_table_names as $target_table_name ) {
						...
						'</option>';
						?>
					jQuery('#relation_table_name_' + row_num).append("<?php echo $option; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
				...
				function add_column(index, selected_value = '') {
					var source_column_name_list =
						...
						foreach ( $source_table_columns as $column ) {
							?>
						'<option value="<?php echo esc_attr( $column['column_name'] ); ?>"><?php echo esc_attr( $column['column_name'] ); ?></option>' +
							<?php
						}
						?>
						'</select>';
				...
				<fieldset class="wpda_fieldset wpdp_fieldset">
					...
					<table class="wpda-table-structure">
						...
						<tr>
							...
							<th>
								<?php echo __( 'Visible', 'wp-data-access' ); ?>
							</th>
							<th>
								<?php echo __( 'Label', 'wp-data-access' ); ?>
							</th>
							<th></th>
							<th>
												...
												<select name="<?php echo esc_attr( $column_name ); ?>_lookup">
													...
													foreach ( $lookup_column_list as $lookup_column ) {
														...
														<option value="<?php echo esc_attr( $lookup_column['column_name'] ); ?>"
															...
															?>
														>
															<?php echo esc_attr( $lookup_column['column_name'] ); ?>
														</option>
														<?php
													}
													?>
				...
				<fieldset class="wpda_fieldset wpdp_fieldset">
					...
					<table class="wpda-table-structure">
						...
						<tr>
							...
							<th>
								<?php echo __( 'Read only', 'wp-data-access' ); ?>
							</th>
							<th>
						...
						foreach ( $structure as $column ) {
							...
							<tr id="tableform_<?php echo esc_attr( $i ); ?>">
								...
								<td>
									<?php echo esc_attr( $data_type ); ?>
									<?php
									echo $this->wpda_list_columns->get_auto_increment_column_name() === $column_name ?
										' (auto increment)' : '';
									?>
								</td>
								<td>
									<?php echo 'Yes' === $key ? 'Yes' : ''; ?>
								</td>
								<td>
									<?php echo 'Yes' === $mandatory ? 'Yes' : ''; ?>
								</td>
								<td>
									<?php
									...
									<input type="checkbox"
										...
										?>
										   style="vertical-align:middle;width:16px;height:16px;"
									/>
									<?php if ( 'Yes' === $l_key || 'Yes' === $l_mandatory ) { ?>
								...
								<td>
									<input type="checkbox" name="<?php echo esc_attr( $column_name ); ?>_readonly"
										   style="vertical-align:middle;width:16px;height:16px;"
											<?php
