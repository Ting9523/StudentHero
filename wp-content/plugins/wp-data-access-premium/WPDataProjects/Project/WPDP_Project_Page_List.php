	...
	class WPDP_Project_Page_List extends WPDP_Child_List_Table {
		...
		public function column_default( $item, $column_name ) {
			if ( 'page_id' === $column_name ) {
				...
				if ( '' !== $warning ) {
					$content = $item['page_id'] . $warning .
						substr( parent::column_default( $item, $column_name ), strlen( $item['page_id'] ) );
		...
		protected function column_default_add_action( $item, $column_name, &$actions ) {
			...
			if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
				$link_title = 'Get shortcodes';
				$title      = 'Shortcodes';
			}
			?>
			<div id="wpda_project_<?php echo esc_attr( $item['page_id'] ); ?>"
				 title="<?php echo __( 'Shortcodes', 'wp-data-access' ); ?>"
				 style="display:none"
			>
				...
				if ( wpda_freemius()->can_use_premium_code__premium_only() ) {
					...
					<p class="wpda_shortcode_text">
						<strong>
							[wpdadataforms project_id="<?php echo esc_attr( $item['project_id'] ); ?>" page_id="<?php echo esc_attr( $item['page_id'] ); ?>"]
						</strong>
					</p>
					<p class="wpda_shortcode_buttons">
						<button class="button wpda_shortcode_clipboard wpda_shortcode_button"
								type="button"
								data-clipboard-text='[wpdadataforms project_id="<?php echo esc_attr( $item['project_id'] ); ?>" page_id="<?php echo esc_attr( $item['page_id'] ); ?>"]'
				...
				<p class="wpda_shortcode_text">
					<strong>
						[wpdadiehard project_id="<?php echo esc_attr( $item['project_id'] ); ?>" page_id="<?php echo esc_attr( $item['page_id'] ); ?>"]
					</strong>
				</p>
				<p class="wpda_shortcode_buttons">
					<button class="button wpda_shortcode_clipboard wpda_shortcode_button"
							type="button"
							data-clipboard-text='[wpdadiehard project_id="<?php echo esc_attr( $item['project_id'] ); ?>" page_id="<?php echo esc_attr( $item['page_id'] ); ?>"]'
							onclick="jQuery.notify('<?php echo __( 'Shortcode successfully copied to clipboard!' ); ?>','info')"
					>
						<?php echo __( 'Copy', 'wp-data-access' ); ?>
					</button>
				...
				<?php
				$shortcode_enabled =
					'on' === WPDA::get_option( WPDA::OPTION_PLUGIN_WPDADIEHARD_POST ) &&
					'on' === WPDA::get_option( WPDA::OPTION_PLUGIN_WPDADIEHARD_PAGE );
